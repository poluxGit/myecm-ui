...
class CategorieView extends React.Component {
  ...
  componentWillMount() {
    ...
    // EDIT COMMAND!
    this.pubsub_token_editCat = pubsub.subscribe('app-cat-edit', function(eventName,catId) {
      console.log('AppMyDocsContainer - Event "'+eventName+'" on "'+catId+'" received.');
      var lArrCatData = LocalData.getCategorieById(catId);
      this._updateFieldValueInState(lArrCatData);
  ...
  /*
   * CallBack CREATE Submit Form Event
   */
  ...
  /*
   * CallBack UPDATE Submit Form Event
   */
  _cbSubmitFormEventUpdate(response) {

    if(response.status == 200)
    {
      pubsub.publish('app-message', {type:'success',message:'Catégorie (id:"'+response.data+'") mise à jour avec succès.'});
      this.close();
    }
    else {
      pubsub.publish('app-message', {type:'error',message:'Error (Code:"'+response.status+'"|Message:"'+response.message+'"|Data:"'+JSON.stringify(response.data)+'").'});
  ...
  render() {
    ...
    return (
        <Modal show={this.state.showModal} onHide={this.close}>
          <Modal.Header>
            <Modal.Title>{this.state.isNew? "Nouvelle Catégorie":"Catégorie #"+this.state.cat_data.cat_id}</Modal.Title>
          </Modal.Header>
